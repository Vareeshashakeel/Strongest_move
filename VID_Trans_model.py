...
class VID_Trans(nn.Module):
    def __init__(self, num_classes, camera_num=None, pretrainpath=None, xcam_block_indices=(5, 8), xcam_use_patch_mean=True):
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None):
        ...
        features, intermediate_tokens = self.base(x, return_intermediate=True, out_indices=self.xcam_block_indices)
        ...
        xcam_feats = []
        for _, token_tensor in intermediate_tokens:
            xcam_feats.append(self._aggregate_temporal_feature(token_tensor, b, t))
    ...
    def load_param(self, trained_path, load=False):
        if not load:
            ...
            for i in param_dict:
                if i in self.state_dict() and 'classifier' not in i and 'sie_embed' not in i:
